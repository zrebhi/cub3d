...
static int fill_window(t_window *window, t_cub *cub)
{
	...
	while (y < W_HEIGHT)
	{
		...
		while (x < W_WIDTH)
		{
			line_len = (int) ft_strlen(cub->map[y * (cub->map_height) / W_HEIGHT]);
			if (x * line_len / W_WIDTH > cub->map_mwidth)
				put_pxl_img(&img, x, y, 0);
			else if (ft_isalpha(cub->map[y * (cub->map_height) / W_HEIGHT][x * (cub->map_mwidth) / W_WIDTH]))
				put_pxl_img(&img, x, y, 0x00FF00BB);
			else if (cub->map[y * (cub->map_height) / W_HEIGHT][x * (cub->map_mwidth) / W_WIDTH] != '0')
				put_pxl_img(&img, x, y, 0);
			else
				put_pxl_img(&img, x, y, 16777215);
			x++;
		}
		y++;
	}
